...
/*
void mytask(unsigned long arg)
{
    struct key_desc * ptr_keyds = (struct key_desc *) arg;
...
void mywork_func(struct work_struct * ptr)
{
    key_dev2->key_cond = 1;
	wake_up_interruptible(&key_dev2->wq_head);
    //kill_fasync(&key_dev2->fasync_s,SIGIO,POLLIN);
}
...
irqreturn_t key_irq_handler(int irqno, void *devid)
{
	...
	//tasklet_schedule(&key_dev2->mytasklet);
    schedule_work(&key_dev2->mywork);
    return IRQ_HANDLED;
}
...
static int __init key_drv_init(void)
{
	...
	//tasklet_init(&key_dev2->mytasklet, mytask,(unsigned long)key_dev2);
    INIT_WORK(&key_dev2->mywork,mywork_func);
