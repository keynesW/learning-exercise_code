...
int register_exynosadc_chrdev(struct platform_device *pdev)
{
	...
	//获取adc clock  Documentation/devicetree/bindings/clock/exynos4-clock.txt
	padc->adc_clk = clk_get(&pdev->dev, "adc");// driver/device.c
...
int exynosadc_probe(struct platform_device *pdev)
{
	...
	//占用IO资源，请求使用IO资源
	//__request_region检查是否可以安全占用起始物理地址start
	//之后的连续n字节大小空间
