...
void list_quene_Init(list_quene_mk_st ** p_list_quene,int max_que)
{
    *p_list_quene = (list_quene_mk_st *) malloc( sizeof (list_quene_mk_st));
    ...
    (*p_list_quene)->max_que = max_que;
    (*p_list_quene)->cunt = 0;

}

void add2_list_quene(list_quene_mk_st * p_list_quene,data_t data)
{
        ...
        p_list_quene->rear->next = temp;
        //printf("addr of temp->next = %p\n",temp->next);
        p_list_quene->rear = temp;    /////
        p_list_quene->cunt ++;
}

data_t get_from_list_quene(list_quene_mk_st * p_list_quene)
{
