...
void list_Init(link_list ** ptr_list_head )
{
    *ptr_list_head  = (link_list *)malloc(sizeof (link_list));
    (*ptr_list_head)->prior = *ptr_list_head;
...
void list_Insert_head(link_list * ptr_list_head ,int data)
{
    ...
    ptr_new_node->prior = ptr_list_head;
...
void del_node(link_list * ptr_list_head,int data)
{
    ...
    {   
        while(Iterator->next)
        {
            ...
            {
                link_list *temp_next = NULL;
                link_list *temp_prior = NULL;
                temp_next = Iterator->next;
                temp_prior = Iterator->prior;
                free(Iterator);
                
                temp_prior->next = temp_next;
                tem_next->prior = temp_prior;

            }else
            {
                if(Iterator == ptr_list_head)
                    Iterator->next = NULL; 
                else
